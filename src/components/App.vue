<template>
  <div>
    <div id="loading-overlay" v-show="loading.show">
      <div id="loading-content">
        <div v-html="loading.message"></div>
        <progress-bar :width="loading.width"></progress-bar>
      </div>
    </div>

    <div class="de-center de-header1">
      <div class="de-width-headers" :class="{ 'width-IE': isIE }">
        Dashboard Powered By Web Worker
      </div>
      <div id="ie-message" v-if="isIE">
        <div><i class="material-icons">error</i></div>
        <div>
          Some features on this dashboard are not available on IE. For better
          performance, please use Chrome. (go/Chrome)
        </div>
      </div>
    </div>

    <div class="de-center de-header2">
      <div class="de-width-headers"><area-list></area-list></div>
    </div>

    <div class="de-center de-header3"><de-tabs></de-tabs></div>
...
export default {
  ...
  computed: {
    ...
    ...Vuex.mapGetters(["getStages", "getStatuses"]),
    isIE() {
  ...
  methods: {
    ...
    addGlidePathsToBizDeliverables() {
      ...
      let BDS = this.bizDeliverables;

      for (let i = 0; i < this.DEGPIterations.length; i++) {
        //filter Glidepaths by ID
        glidePathsByIteration = this.DEGPGlidePaths.filter(item => {
          return this.DEGPIterations[i].ID == item.Iteration_x0020_TitleId;
        });
        for (let g = 0; g < glidePathsByIteration.length; g++) {
          for (let bd of BDS) {
            if (bd.ID == glidePathsByIteration[g].NumberingId) {
              bd = Object.assign(bd, {
                Iteration_ID: this.DEGPIterations[i].ID,
                Iteration_End_Date: this.DEGPIterations[i]
                  .Documentation_x0020_End_x0020_Da,
                Iteration_Start_Date: this.DEGPIterations[i]
                  .Documentation_x0020_Start_x0020_,
                Iteration_Year: this.DEGPIterations[i].Plan_x0020_Year
              });
            }
          }
        }
    ...
    refreshStatuses() {
      ...
      this.fetchStatuses()
        .then(() => {
          this.setLoading({
            show: true,
            message: "Rebuilding Data Structures...",
            width: 50
          });
    ...
    startWorker(activities, areas, bds, stages, statuses) {
      if (typeof Worker !== "undefined") {
        ...
        this.webWorker.postMessage([activities, areas, bds, stages, statuses]);
        let that = this;
