<template>
  <div>
    <select
      style="display:inline; width:360px"
      @change="updateArea(selected);"
      v-model="selected"
    >
      <option
...
export default {
  ...
  computed: {
    ...Vuex.mapState(["areas"]),
    ...Vuex.mapGetters(["getSelectedStage", "getStages"])
  },
  ...
  methods: {
    ...
    setDefaultArea(area) {
      ...
      if (navigator.userAgent.indexOf("MSIE") > -1 && areaID == -1) {
    ...
    updateArea(selected) {
      ...
      this.$router.push({
        query: { area: this.selected, stage: this.getSelectedStage }
      });
    }
  },
