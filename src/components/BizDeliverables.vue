<template>
  <div>
    <div
      class="de-content-row"
      v-for="(value, key) in deliverablesFiltered"
      :key="key"
    >
      <div
        class="de-content-left"
        :class="{
          'biz-deliverable': bizDeliverable(value),
          'member-experience': !bizDeliverable(value)
        }"
      >
        <table class="bd-table">
          <tr>
            <td>
              <a
                class="bd-link"
                @click="passToDeds(value.Numbering, value.Title);"
                >{{ value.Numbering }} {{ value.Title }}</a
              >
            </td>
          </tr>
        </table>

        <div>{{ showBDPercents[key].percent }}</div>
      </div>

      <div
        class="vertical glidedates"
        v-if="value.Iteration_Start_Date && getSelectedStage == 3"
      >
...
export default {
  ...
  computed: {
    ...
    ...Vuex.mapGetters(["getFromRoute", "getSelectedStage"]),

    deliverablesFiltered() {
      let areaID = this.selectedArea;
      let filterByArea = this.bizDeliverables;

      if (this.selectedArea == 2018) {
        //TODO: careful with strings, may need to parseInt
        filterByArea = this.bizDeliverables.filter(item => {
          return item.Iteration_Year == this.selectedArea;
        });
      } else if (this.selectedArea != -1) {
        //Careful with query params that are strings, don't use !==
        filterByArea = this.bizDeliverables.filter(item => {
    ...
    showBDPercents() {
      ...
      let filterByAreaStage = this.bdPercents.filter(item => {
