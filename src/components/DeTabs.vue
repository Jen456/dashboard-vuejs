<template>
  <div class="de-tabs de-width-headers de-tab-menus">
    <div
      class="de-tab"
      :class="{ selected: value.ID === getSelectedStage }"
      v-for="(value, key) in getStages"
      :key="key"
      @click="setStage(value.ID);"
      @mouseover="setStageHover(value.ID);"
      @mouseout="setStageHover(-1);"
    >
      <div>{{ value.Title }}</div>
      <div>
        <pie-chart-enter
          :index="key"
          :filterStagePercents="filterStagePercents"
        ></pie-chart-enter>
      </div>
...
export default {
  ...
  computed: {
    ...Vuex.mapState(["selectedArea"]),
    ...Vuex.mapGetters(["getSelectedStage", "getStages", "getStagePercents"]),
  ...
  methods: {
    ...
    setStage(id) {
      ...
      this.$router.push({ query: { area: this.selectedArea, stage: id } });
    }
  },
